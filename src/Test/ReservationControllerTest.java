...
public class ReservationControllerTest {
    public static void runTests() {
        ...
        Book book1 = new Book("W pustyni i w puszczy", "Przygoda", "Sienkiewicz");
        Book book2 = new Book("Krzyżacy", "Przygoda", "Sienkiewicz");
        book1.setSector(sector);
        ...
        // 1. Udana rezerwacja dwóch książek
        try {
            reservationController.reserveBook(
                    Arrays.asList(book1, book2), client, LocalDate.now(), LocalDate.now().plusDays(7));
        ...
        // 2. Próba ponownej rezerwacji jednej z tych samych książek (powinno się nie udać)
        try {
            reservationController.reserveBook(
                    List.of(book1), client, LocalDate.now(), LocalDate.now().plusDays(5));
        ...
        Book book3 = new Book("Stara księga", "Fantasy", "Author");
        book3.setSector(sector);
        try {
            reservationController.reserveBook(
                    List.of(book3), client, LocalDate.now().plusDays(10), LocalDate.now());
        ...
        // 1. Tworzymy przeterminowaną rezerwację (jedna książka, przeszła data)
        Book expiredBook = new Book("Bardzo stara księga", "Fantasy", "Author");
        expiredBook.setSector(sector);
        try {
            reservationController.reserveBook(
                    List.of(expiredBook), client, LocalDate.now().minusDays(10), LocalDate.now().minusDays(7));
